...
def ensure_cuda_dll_paths() -> None:
    ...
    submods = ["cuda_runtime", "cublas", "cudnn", "cufft", "curand", "cusolver", "cusparse", "cuda_nvcc"]
...
def configure_hardware(
    ...
    enable_xla = os.getenv("TOTO_ENABLE_XLA", "").strip().lower() in {"1", "true", "yes", "y"}
    ptxas_path = shutil.which("ptxas.exe") or ""
    hw["ptxas_path"] = ptxas_path
    has_ptxas = bool(ptxas_path)
    has_libdevice = False
    if has_ptxas:
        try:
            ptxas_bin = Path(ptxas_path).resolve().parent
            cuda_data_dir = ptxas_bin.parent
            libdevice = cuda_data_dir / "nvvm" / "libdevice" / "libdevice.10.bc"
            if libdevice.exists():
                has_libdevice = True
                hw["cuda_data_dir"] = str(cuda_data_dir)
        except Exception:
            has_libdevice = False
    # Keep XLA opt-in for stability. Enable only if explicitly requested.
    if disable_xla or (not enable_xla) or not has_ptxas or not has_libdevice:
        hw["xla"] = False
    else:
        try:
...
def build_combined_distributions(
    ...
    for i in range(1, MODEL_WIN_SLOT_COUNT + 1):
        key = f"win_{i}"
...
def addl_repetition_penalty_score(
    key: Tuple[int, ...],
    recent_pred_addl_sets: List[Tuple[int, ...]] | None,
    anti_repeat_window: int = 10,
    anti_repeat_lambda: float = 0.85,
) -> float:
    if not recent_pred_addl_sets:
        return 0.0
    ...
    hist = recent_pred_addl_sets[-window:]
    penalty = 0.0
    ...
    for rank, prev in enumerate(reversed(hist), start=1):
        ...
        if key == prev:
            penalty += 1.90 * decay
        elif overlap_n >= max(1, len(key) - 1):
            penalty += 1.10 * decay
        elif overlap_n >= 1:
            penalty += 0.38 * decay
    idx = np.asarray([int(x) - 1 for x in key], dtype=np.int32)
    freq_norm = num_freq / (float(np.max(num_freq)) + 1e-12)
    penalty += 0.35 * float(np.mean(freq_norm[idx]))
    return lam * penalty


def choose_addl_from_probs(
    combined_addl: np.ndarray,
    win_numbers: List[int],
    count: int = PRED_ADDL_COUNT,
    recent_pred_addl_sets: List[Tuple[int, ...]] | None = None,
    anti_repeat_window: int = 10,
    anti_repeat_lambda: float = 0.85,
) -> List[int]:
    n_pick = int(max(1, count))
    blocked = set(int(x) for x in win_numbers)
    ordered = [int(i + 1) for i in np.argsort(combined_addl)[::-1].tolist() if int(i + 1) not in blocked]
    if not ordered:
        return [int(np.argmax(combined_addl) + 1)]

    top_pool_n = int(min(len(ordered), max(n_pick + 10, 14)))
    top_pool = ordered[:top_pool_n]
    cand_sets: List[Tuple[int, ...]] = []
    for comb in itertools.combinations(top_pool, n_pick):
        cand_sets.append(tuple(sorted(int(x) for x in comb)))

    if not cand_sets:
        base = tuple(sorted(int(x) for x in ordered[:n_pick]))
        cand_sets = [base]

    best_key = cand_sets[0]
    best_score = -1e18
    for key in cand_sets:
        idx = np.asarray([int(x) - 1 for x in key], dtype=np.int32)
        p = np.clip(combined_addl[idx], 1e-12, 1.0)
        score = float(np.mean(np.log(p)))
        score -= addl_repetition_penalty_score(
            key=key,
            recent_pred_addl_sets=recent_pred_addl_sets,
...
def candidate_feature_vector(
    ...
    half_target = float(len(arr)) / 2.0 if len(arr) > 0 else 0.0
    odd_balance = abs(float(np.sum(arr % 2 == 1)) - half_target) / max(1.0, half_target)
    low_balance = abs(float(np.sum(arr <= 24)) - half_target) / max(1.0, half_target)
    return np.asarray(
        [
            float(np.mean(logp)),
            float(np.min(logp)),
...
def generate_candidate_sets(
    ...
    if rng is None:
        top_idx = np.argsort(combined_win)[-max(PRED_WIN_COUNT + 2, 8) :].astype(np.int64)
    ...
    cands: List[List[int]] = []
    cands.append(sorted([int(i + 1) for i in np.argsort(combined_win)[-PRED_WIN_COUNT:].tolist()]))
    cands.append(select_win_numbers_with_repulsion(combined_win, count=PRED_WIN_COUNT, p1=0.93, p2=0.97, p3=0.985))
    cands.append(select_win_numbers_with_repulsion(combined_win, count=PRED_WIN_COUNT, p1=0.88, p2=0.94, p3=0.97))
    cands.append(select_win_numbers_with_repulsion(combined_win, count=PRED_WIN_COUNT, p1=0.83, p2=0.90, p3=0.95))
    for temp in (0.78, 0.88, 1.05, 1.22):
        tprob = temperature_scale_prob(combined_win, temperature=temp)
        cands.append(sorted([int(i + 1) for i in np.argsort(tprob)[-PRED_WIN_COUNT:].tolist()]))
    for _ in range(max(0, int(candidate_gumbel_samples))):
        cands.append(gumbel_topk_set(combined_win, k=PRED_WIN_COUNT, rng=rng))

    out: List[List[int]] = []
    seen: set[Tuple[int, ...]] = set()
    for c in cands:
        key = tuple(sorted(int(x) for x in c))
        if len(key) != PRED_WIN_COUNT:
            continue
        if key in seen:
            continue
        seen.add(key)
...
def repetition_penalty_score(
    ...
    for rank, prev in enumerate(reversed(hist), start=1):
        ...
        prev_set = set(int(x) for x in prev)
        overlap_n = len(key_set.intersection(prev_set))
        overlap = overlap_n / max(1.0, float(len(key_set)))
        for n in prev_set:
            num_freq[int(n) - 1] += decay
        if key == prev:
            penalty += 2.20 * decay
        elif overlap_n >= max(5, PRED_WIN_COUNT - 1):
            penalty += (1.45 + 0.35 * overlap_n) * decay
        elif overlap_n >= max(4, PRED_WIN_COUNT // 2):
            penalty += 1.65 * decay
        elif overlap_n >= max(3, PRED_WIN_COUNT // 3):
            penalty += 0.62 * decay
        elif overlap_n >= max(2, PRED_WIN_COUNT // 4):
            penalty += 0.18 * decay
        else:
            penalty += 0.06 * max(0.0, overlap - (1.0 / max(1.0, float(PRED_WIN_COUNT)))) * decay
...
def combine_prediction(
    ...
    if recent_pred_sets:
        ...
        if strict_mode:
            max_last_overlap = max(2, int(round(PRED_WIN_COUNT * 0.30)))
            max_hist_overlap = max(3, int(round(PRED_WIN_COUNT * 0.40)))
        else:
            max_last_overlap = max(3, int(round(PRED_WIN_COUNT * 0.40)))
            max_hist_overlap = max(4, int(round(PRED_WIN_COUNT * 0.50)))
    ...
    addl_numbers = choose_addl_from_probs(
        combined_addl=combined_addl,
        win_numbers=win_numbers,
        count=PRED_ADDL_COUNT,
        recent_pred_addl_sets=recent_pred_addl_sets,
        anti_repeat_window=anti_repeat_window,
        anti_repeat_lambda=max(0.35, 0.72 * float(anti_repeat_lambda)),
    )
    return win_numbers, addl_numbers


def summarize_hits(pred_win: List[int], pred_addl: List[int] | int, actual_win: List[int], actual_addl: int) -> Tuple[int, int]:
    hit_count = len(set(pred_win).intersection(actual_win))
    if isinstance(pred_addl, (list, tuple, set, np.ndarray)):
        addl_set = set(int(x) for x in pred_addl)
    else:
        addl_set = {int(pred_addl)}
    addl_hit = 1 if int(actual_addl) in addl_set else 0
...
def evaluate_hit_score(
    ...
    for local_i, seq_i in enumerate(eval_idx):
        ...
        recent_pred_addl_sets.append(tuple(sorted(int(x) for x in pred_addl)))
...
def build_near_miss_variants(
    ...
    out: List[List[int]] = []
    if len(base) != PRED_WIN_COUNT:
        return out
    cursor = 0
    for replace_pos in range(len(base)):
        trial = base.copy()
...
def fit_reward_reranker(
    ...
    for local_i, seq_i in enumerate(eval_use):
        ...
        for cand in cands:
            ...
            if key in seen or len(key) != PRED_WIN_COUNT:
                continue
            seen.add(key)
...
def run_backtest(
    ...
    def _record_from_prediction(
        ...
        if isinstance(pred_addl, (list, tuple, set, np.ndarray)):
            pred_addl_view = " ".join(str(int(x)) for x in sorted(int(x) for x in pred_addl))
        else:
            pred_addl_view = str(int(pred_addl))
        return {
            "fold": fold,
            "target_row": target_row,
            "draw": _row_draw(target_row),
...
def main() -> None:
    ...
    ptxas_note = hw.get("ptxas_path", "")
    if ptxas_note:
        ptxas_note = f" ptxas={ptxas_note}"
    else:
        ptxas_note = " ptxas=NOT_FOUND"
    cuda_data_dir = str(hw.get("cuda_data_dir", "") or "")
    cuda_note = f" cuda_data_dir={cuda_data_dir}" if cuda_data_dir else ""
    print(
        f"Hardware: GPUs={hw['gpu_count']} names={hw['gpu_names']} "
        f"mixed_precision={hw['mixed_precision']} xla={hw['xla']}{ptxas_note}{cuda_note}"
    )
    ...
    if not backtest_df.empty and "pred_addl" in backtest_df.columns:
        rep_addl = backtest_df["pred_addl"].astype(str).value_counts()
        rep_addl = rep_addl[rep_addl > 1]
        if not rep_addl.empty:
            top_rep_addl = rep_addl.head(8).to_dict()
            print(f"[BACKTEST] repeated pred_addl sets detected: {top_rep_addl}")
    print("")
