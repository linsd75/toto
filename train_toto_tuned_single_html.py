...
WIN_BLEND_KEYS = ["model_soft", "model_set", "cluster", "gravity", "transition", "sma"]
ADDL_BLEND_KEYS = ["addl_model", "addl_cluster", "addl_gravity", "addl_sma"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--w-gravity", type=float, default=DEFAULT_FEATURE_GROUP_WEIGHTS["gravity"], help="Feature weight for gravity prior features")
    parser.add_argument("--w-transition", type=float, default=DEFAULT_FEATURE_GROUP_WEIGHTS["transition"], help="Feature weight for transition prior features")
    ...
    parser.add_argument("--w-sma", type=float, default=DEFAULT_FEATURE_GROUP_WEIGHTS["sma"], help="Feature weight for slime-mould aggregate features")
    parser.add_argument("--w-sma-branch", type=float, default=DEFAULT_FEATURE_GROUP_WEIGHTS["sma_branch"], help="Feature weight for slime branch features")
    parser.add_argument("--w-sma-void", type=float, default=DEFAULT_FEATURE_GROUP_WEIGHTS["sma_void"], help="Feature weight for slime void features")
...
def feature_group_weights_from_args(args: argparse.Namespace) -> Dict[str, float]:
    return {
        ...
        "gravity": float(args.w_gravity),
        "transition": float(args.w_transition),
        "repel": float(args.w_repel),
        "cluster": float(args.w_cluster),
        "line": float(args.w_line),
        "sma": float(args.w_sma),
        "sma_branch": float(args.w_sma_branch),
        "sma_void": float(args.w_sma_void),
    }
...
def build_feature_pipeline(
    ...
    gravity_system = build_gravity_system(df)
    gravity_prior_matrix = gravity_system["gravity_prior"]
    transition_prior_matrix = gravity_system["transition_prior"]
    repel_prior_matrix = gravity_system["repel_prior"]

    line_system = build_line_endpoint_system(df)
    line_prior_matrix = line_system["line_prior"]
    line_endpoint_matrix = line_system["line_endpoint"]
    line_merge_matrix = line_system["line_merge"]
    cooc_system = build_cooccurrence_system(df)
    cooc_prior_matrix = cooc_system["cooc_prior"]
    line_merge_norm = line_merge_matrix / (line_merge_matrix.sum(axis=1, keepdims=True) + 1e-8)
    # Transition blend: gravity transition + line trajectory + endpoint merges + co-occurrence flow.
    transition_prior_matrix = (
        0.46 * gravity_system["transition_prior"]
        + 0.22 * line_prior_matrix
        + 0.16 * line_merge_norm
        + 0.16 * cooc_prior_matrix
    )
    transition_prior_matrix = transition_prior_matrix / (transition_prior_matrix.sum(axis=1, keepdims=True) + 1e-8)

    slime_system = build_slime_mould_system(df)
    sma_prior_matrix = slime_system["sma_prior"]
    sma_branch_matrix = slime_system["sma_branch"]
    sma_void_matrix = slime_system["sma_void"]
    ...
    gravity_cols = [f"Gravity_{i+1}" for i in range(49)]
    transition_cols = [f"TransPrior_{i+1}" for i in range(49)]
    ...
    sma_cols = [f"SMA_{i+1}" for i in range(49)]
    sma_branch_cols = [f"SMABranch_{i+1}" for i in range(49)]
    sma_void_cols = [f"SMAVoid_{i+1}" for i in range(49)]

    extra_features = pd.DataFrame(
        np.hstack(
            [
                gravity_prior_matrix,
                transition_prior_matrix,
                line_prior_matrix,
                line_merge_matrix,
                cooc_prior_matrix,
        ...
        columns=gravity_cols + transition_cols + line_cols + line_merge_cols + cooc_cols + repel_cols + cluster_prior_cols + sma_cols + sma_branch_cols + sma_void_cols,
        index=df.index,
    )
    ...
    feature_weights = np.array(
        ...
        + [group_weights["gravity"]] * len(gravity_cols)
        + [group_weights["transition"]] * len(transition_cols)
        + [group_weights["line"]] * len(line_cols)
        + [group_weights["line"]] * len(line_merge_cols)
        + [group_weights["transition"]] * len(cooc_cols)
        + [group_weights["repel"]] * len(repel_cols)
        + [group_weights["cluster"]] * len(cluster_prior_cols)
        + [group_weights["sma"]] * len(sma_cols)
        + [group_weights["sma_branch"]] * len(sma_branch_cols)
        + [group_weights["sma_void"]] * len(sma_void_cols),
        dtype=np.float32,
    )
    ...
    return {
        ...
        "gravity_prior_matrix": gravity_prior_matrix,
        "transition_prior_matrix": transition_prior_matrix,
        "line_prior_matrix": line_prior_matrix,
        "line_endpoint_matrix": line_endpoint_matrix,
        "line_merge_matrix": line_merge_matrix,
        "cooc_prior_matrix": cooc_prior_matrix,
        "repel_prior_matrix": repel_prior_matrix,
        "sma_prior_matrix": sma_prior_matrix,
        "sma_branch_matrix": sma_branch_matrix,
        "sma_void_matrix": sma_void_matrix,
    ...
    for trial_i, cand in enumerate(candidates, start=1):
        try:
            ...
            hit_metrics = evaluate_hit_score(
                ...
                gravity_prior_matrix=pipe["gravity_prior_matrix"],
                transition_prior_matrix=pipe["transition_prior_matrix"],
                sma_prior_matrix=pipe["sma_prior_matrix"],
...
def build_gravity_system(
    df: pd.DataFrame,
    lookback: int = 30,
    decay: float = 0.86,
    tau: float = 4.2,
    repel_window: int = 10,
    group_sigma: float = 2.8,
) -> Dict[str, np.ndarray]:
    """
    Build dynamic attraction/repulsion fields using only historical rows before each target row.

    - gravity_prior: smooth attraction around recent winning/additional numbers with temporal decay.
    - transition_prior: next-number preference conditioned on previous draw numbers.
    - repel_prior: recent frequency pressure (used as repulsive signal).
    - group_field: attraction around contiguous or near-contiguous number groups.
    """
    n = len(df)
    ...
    gravity = np.zeros((n, 49), dtype=np.float32)
    transition = np.zeros((n, 49), dtype=np.float32)
    repel = np.zeros((n, 49), dtype=np.float32)
    group = np.zeros((n, 49), dtype=np.float32)

    # Number-to-number transition counts (Laplace smoothing).
    transition_counts = np.ones((50, 50), dtype=np.float32)

    for t in range(n):
        # Transition prior: based on previous draw numbers and all historical transitions up to t-1.
        if t > 0:
            prev_all = draw_all[t - 1]
            score = np.zeros(49, dtype=np.float32)
            for p in prev_all:
                score += transition_counts[p, 1:]
            transition[t] = score / (score.sum() + 1e-8)

        # Gravity attraction from up to `lookback` past draws.
        max_lag = min(lookback, t)
        for lag in range(1, max_lag + 1):
            w = decay ** (lag - 1)
            past_all = draw_all[t - lag]
            for x in past_all:
                dist = np.abs(num_axis - float(x))
                gravity[t] += w * np.exp(-(dist * dist) / (2.0 * tau * tau))

        # Group attraction from previous draw's near-consecutive groups (captures 29,30,31-like effects).
        if t > 0:
            prev_sorted = np.sort(draw_win[t - 1])
            curr_group: List[int] = [int(prev_sorted[0])]
            groups: List[List[int]] = []
            for x in prev_sorted[1:]:
                if int(x) - curr_group[-1] <= 2:
                    curr_group.append(int(x))
                else:
            ...
            for g in groups:
                if len(g) >= 2:
                    ...
                    strength = 0.6 + 0.55 * len(g)
                    dist = np.abs(num_axis - center)
                    group[t] += strength * np.exp(-(dist * dist) / (2.0 * group_sigma * group_sigma))

        # Repulsion: recent-frequency pressure (numbers that appear too frequently in short window).
        if t > 0:
            start = max(0, t - repel_window)
            recent = draw_win[start:t].reshape(-1)
            counts = np.bincount(recent, minlength=50)[1:].astype(np.float32)
            repel[t] = counts / max(1.0, float(len(recent)))

        # Update transition counts after creating priors for current row.
        if t > 0:
            prev_all = draw_all[t - 1]
            curr_all = draw_all[t]
            for p in prev_all:
                transition_counts[p, curr_all] += 1.0

    gravity = gravity / (gravity.sum(axis=1, keepdims=True) + 1e-8)
    group = group / (group.sum(axis=1, keepdims=True) + 1e-8)
    repel = repel / (repel.sum(axis=1, keepdims=True) + 1e-8)

    gravity_prior = 0.58 * gravity + 0.27 * group + 0.15 * transition
    gravity_prior = gravity_prior / (gravity_prior.sum(axis=1, keepdims=True) + 1e-8)

    return {
        "gravity_prior": gravity_prior.astype(np.float32),
        "transition_prior": transition.astype(np.float32),
        "repel_prior": repel.astype(np.float32),
        "group_field": group.astype(np.float32),
    }
...
def build_slime_mould_system(
    df: pd.DataFrame,
    decay: float = 0.84,
    crowd_window: int = 12,
    branch_strength: float = 0.58,
    crowd_penalty: float = 0.42,
) -> Dict[str, np.ndarray]:
    """
    Slime-mould-inspired adaptive field over numbers 1..49.
    Numbers are treated as living nodes that grow branches (co-occurrence flow)
    and avoid crowded voids (recent over-occupation pressure).
    """
    n = len(df)
    draw_win = df[WIN_COLS].values.astype(np.int32)
    draw_all = df[WIN_COLS + ["Addl No."]].values.astype(np.int32)

    field = np.zeros((n, 49), dtype=np.float32)
    branch = np.zeros((n, 49), dtype=np.float32)
    void = np.zeros((n, 49), dtype=np.float32)

    # Dynamic network: co-occurrence branch graph between numbers.
    graph = np.ones((49, 49), dtype=np.float64)
    trail = np.ones(49, dtype=np.float64) / 49.0

    def spread(vec: np.ndarray) -> np.ndarray:
        out = vec.copy()
        out[:-1] += 0.55 * vec[1:]
        out[1:] += 0.55 * vec[:-1]
        out[:-2] += 0.30 * vec[2:]
    ...
    for t in range(n):
        field[t] = (trail / (trail.sum() + 1e-12)).astype(np.float32)

        if t == 0:
            # initialize with first observed draw
            nums = draw_all[t] - 1
            for a in nums:
                for b in nums:
                    graph[a, b] += 1.0
        ...
        prev = draw_all[t - 1] - 1
        prev_mask = np.zeros(49, dtype=np.float64)
        prev_mask[prev] = 1.0

        # Branch growth: graph-driven nutrient routing + local spread.
        branch_vec = graph @ prev_mask
        branch_vec = spread(branch_vec)
        branch_vec = branch_vec / (branch_vec.sum() + 1e-12)
        branch[t] = branch_vec.astype(np.float32)

        # Void pressure: recent occupancy crowding.
        start = max(0, t - crowd_window)
        recent = draw_win[start:t].reshape(-1) - 1
        crowd = np.bincount(recent, minlength=49).astype(np.float64)
        crowd = crowd / (crowd.sum() + 1e-12)
        void[t] = crowd.astype(np.float32)

        # Slime mould update (adaptive contract-expand flow).
        attract = 0.62 * prev_mask + branch_strength * branch_vec
        repel = crowd_penalty * crowd
        trail = decay * trail + attract - repel
        trail = np.clip(trail, 1e-12, None)
        trail /= trail.sum()

        # Update branch graph with current draw connectivity.
        nums = draw_all[t] - 1
        for a in nums:
            for b in nums:
                if a != b:
    ...
    return {
        "sma_prior": field.astype(np.float32),
        "sma_branch": branch.astype(np.float32),
        "sma_void": void.astype(np.float32),
    }
...
def combine_prediction(
    ...
    # Hybrid gravity blend: attraction (model+gravity+transition+cluster) minus repulsion.
    combined_win = (
        weights["model_soft"] * normalize_prob(win_soft)
        + weights["model_set"] * win_set_prob
        + weights["cluster"] * normalize_prob(win_cluster_prior)
        + weights["gravity"] * normalize_prob(gravity_prior)
        + weights["transition"] * normalize_prob(transition_prior)
        + weights["sma"] * normalize_prob(sma_prior)
    ...
    transition_prob = normalize_prob(transition_prior)
    cluster_prob = normalize_prob(win_cluster_prior)
    sma_prob = normalize_prob(sma_prior)
    ...
    trans_mix = normalize_prob(0.80 * combined_win + 0.20 * transition_prob)
    trans_top = sorted([int(i + 1) for i in np.argsort(trans_mix)[-6:].tolist()])

    def candidate_utility(nums: List[int]) -> float:
        idx = np.asarray(nums, dtype=np.int32) - 1
        p = np.clip(combined_win[idx], 1e-12, 1.0)
        t = np.clip(transition_prob[idx], 1e-12, 1.0)
        c = np.clip(cluster_prob[idx], 1e-12, 1.0)
        s = np.clip(sma_prob[idx], 1e-12, 1.0)
        # Tail-hit mode prefers concentrated high-probability sets.
        util = (
            2.7 * float(np.mean(np.log(p)))
            + 0.42 * float(np.mean(np.log(t)))
            + 0.32 * float(np.mean(np.log(c)))
            + 0.18 * float(np.mean(np.log(s)))
        )
        return util

    candidates: Dict[Tuple[int, ...], float] = {}
    for cand in (plain_top, repel_light, repel_std, trans_top):
    ...
    combined_addl = (
        ...
        + weights["addl_gravity"] * normalize_prob(gravity_prior)
        + weights["addl_sma"] * normalize_prob(sma_prior)
    )
...
def evaluate_hit_score(
    ...
    gravity_prior_matrix: np.ndarray,
    transition_prior_matrix: np.ndarray,
    sma_prior_matrix: np.ndarray,
    ...
    for local_i, seq_i in enumerate(eval_idx):
        ...
        pred_win, pred_addl = combine_prediction(
            ...
            gravity_prior=gravity_prior_matrix[target_row],
            transition_prior=transition_prior_matrix[target_row],
            sma_prior=sma_prior_matrix[target_row],
...
def generate_blend_candidates(base: Dict[str, float]) -> List[Dict[str, float]]:
    ...
    variants = [
        # model-heavy
        {"model_soft": 0.62, "model_set": 0.70, "cluster": 0.12, "gravity": 0.12, "transition": 0.08, "sma": 0.10, "repel": 0.04, "addl_model": 0.80, "addl_cluster": 0.13, "addl_gravity": 0.05, "addl_sma": 0.04},
        # gravity-heavy
        {"model_soft": 0.44, "model_set": 0.48, "cluster": 0.20, "gravity": 0.30, "transition": 0.18, "sma": 0.24, "repel": 0.10, "addl_model": 0.58, "addl_cluster": 0.20, "addl_gravity": 0.18, "addl_sma": 0.12},
        # balanced
        {"model_soft": 0.52, "model_set": 0.58, "cluster": 0.16, "gravity": 0.22, "transition": 0.14, "sma": 0.18, "repel": 0.08, "addl_model": 0.70, "addl_cluster": 0.17, "addl_gravity": 0.11, "addl_sma": 0.09},
        # high transition
        {"model_soft": 0.48, "model_set": 0.54, "cluster": 0.14, "gravity": 0.18, "transition": 0.26, "sma": 0.14, "repel": 0.08, "addl_model": 0.66, "addl_cluster": 0.16, "addl_gravity": 0.14, "addl_sma": 0.10},
        # low repel
        {"model_soft": 0.50, "model_set": 0.56, "cluster": 0.18, "gravity": 0.22, "transition": 0.12, "sma": 0.20, "repel": 0.02, "addl_model": 0.69, "addl_cluster": 0.19, "addl_gravity": 0.10, "addl_sma": 0.11},
        # slime-first
        {"model_soft": 0.46, "model_set": 0.50, "cluster": 0.14, "gravity": 0.18, "transition": 0.10, "sma": 0.36, "repel": 0.08, "addl_model": 0.64, "addl_cluster": 0.14, "addl_gravity": 0.10, "addl_sma": 0.16},
    ]
    for v in variants:
...
def calibrate_blend_weights_by_component_performance(
    ...
    win_keys = ["model_soft", "model_set", "cluster", "gravity", "transition", "sma"]
    addl_keys = ["addl_model", "addl_cluster", "addl_gravity", "addl_sma"]
    rows: List[Dict[str, object]] = []
            ...
            for cand in generate_backtest_blend_candidates(blend_weights, num_random=local_random):
                ...
                for item in payloads:
                    pred_win, pred_addl = combine_prediction(
                        ...
                        gravity_prior=item["gravity_prior"],
                        transition_prior=item["transition_prior"],
                        sma_prior=item["sma_prior"],
...
<body>
  ...
  <div class="card">
    ...
    <div class="kpi">
      ...
      <div class="pill"><strong>Gravity</strong><br>{feature_group_weights["gravity"]:.4f}</div>
      <div class="pill"><strong>Transition</strong><br>{feature_group_weights["transition"]:.4f}</div>
      ...
      <div class="pill"><strong>SMA</strong><br>{feature_group_weights["sma"]:.4f}</div>
      <div class="pill"><strong>SMA Branch</strong><br>{feature_group_weights["sma_branch"]:.4f}</div>
      <div class="pill"><strong>SMA Void</strong><br>{feature_group_weights["sma_void"]:.4f}</div>
    </div>
  ...
  <div class="card">
    ...
    <div class="kpi">
      ...
      <div class="pill"><strong>gravity</strong><br>{calibrated_blend["gravity"]:.6f}</div>
      <div class="pill"><strong>transition</strong><br>{calibrated_blend["transition"]:.6f}</div>
      <div class="pill"><strong>sma</strong><br>{calibrated_blend["sma"]:.6f}</div>
      ...
      <div class="pill"><strong>addl_gravity</strong><br>{calibrated_blend["addl_gravity"]:.6f}</div>
      <div class="pill"><strong>addl_sma</strong><br>{calibrated_blend["addl_sma"]:.6f}</div>
    </div>
  ...
  <div class="card">
    <h2>Interactive Charts (Grafana Style)</h2>
    ...
    <h3>Long Grid Charts (Scrollable, 2 Rows)</h3>
    <div class="long-rows">
      <div class="long-row">
        <div class="long-chart"><div class="long-scroll"><div id="slime_field" class="plotly-long"></div></div></div>
  ...
  <script>
    ...
    const squareHeatLayout = (title, xTitle, yTitle, yLabels, nCols = 49, maxHeight = 1800) => {{
      ...
      const width = Math.max(560, 170 + cell * nCols + 96);
      return {{
        ...baseLayout,
        title,
        height,
    ...
    Plotly.newPlot('slime_field', [{{
      z: D.slime.field_z,
      x: D.slime.num_axis,
      y: D.slime.row_pos,
      type: 'heatmap',
      colorscale: 'Plasma'
    }}], squareHeatLayout('Slime Mould Field (Branches / Voids)', 'Number (1-49)', 'Recent Draw Rows', D.slime.row_labels, D.slime.num_axis.length, 2100), {{responsive:true}});
    ...
    Plotly.newPlot('occurrence_heat', [{{
      ...
      type: 'heatmap',
      colorscale: 'Greens'
    }}], squareHeatLayout('Winning Number Occupancy Heatmap', 'Number (1-49)', 'Recent Draw Rows', D.slime.row_labels, D.slime.num_axis.length, 2100), {{responsive:true}});
...
def main() -> None:
    ...
    line_endpoint_matrix = pipe["line_endpoint_matrix"]
    line_merge_matrix = pipe["line_merge_matrix"]
    cooc_prior_matrix = pipe["cooc_prior_matrix"]
    repel_prior_matrix = pipe["repel_prior_matrix"]
    sma_prior_matrix = pipe["sma_prior_matrix"]
    sma_branch_matrix = pipe["sma_branch_matrix"]
    sma_void_matrix = pipe["sma_void_matrix"]
    win_cluster_prior_matrix = pipe["win_cluster_prior_matrix"]
    addl_cluster_prior_matrix = pipe["addl_cluster_prior_matrix"]
    ...
    # Build one-step-ahead gravity/transition/repel priors (for row n, based on rows [0..n-1]).
    dummy_next = df.iloc[[-1]].copy()
    gravity_next_df = pd.concat([df, dummy_next], ignore_index=True)
    gravity_next = build_gravity_system(gravity_next_df)
    gravity_next_prior = gravity_next["gravity_prior"][-1]
    transition_next_prior = gravity_next["transition_prior"][-1]
    repel_next_prior = gravity_next["repel_prior"][-1]
    line_next = build_line_endpoint_system(gravity_next_df)
    line_next_prior = line_next["line_prior"][-1]
    line_next_merge = line_next["line_merge"][-1]
    line_next_merge = line_next_merge / (line_next_merge.sum() + 1e-8)
    cooc_next = build_cooccurrence_system(gravity_next_df)["cooc_prior"][-1]
    transition_next_prior = (
        0.46 * transition_next_prior
        + 0.22 * line_next_prior
        + 0.16 * line_next_merge
        + 0.16 * cooc_next
    )
    transition_next_prior = transition_next_prior / (transition_next_prior.sum() + 1e-8)

    slime_next = build_slime_mould_system(gravity_next_df)
    sma_next_prior = slime_next["sma_prior"][-1]
    ...
    pred_win, pred_addl = combine_prediction(
        ...
        gravity_prior=gravity_next_prior,
        transition_prior=transition_next_prior,
        sma_prior=sma_next_prior,
    ...
    dashboard = {
        ...
        "slime": {
            "field_z": sma_prior_matrix[recent_idx].astype(float).tolist(),
